...
export function comprobarApodo(apodo, id, lista) {
    let array = [].concat(lista)
    if (apodo == "") {
        array[0] = false
        array[4] = array[4] + " el pokemon número ", id + 1, " de tu equipo necesita un apodo."
        console.log("El pokemon número ", id, " de tu equipo necesita un apodo")
    }
    return array
}

export function comprobarEvs(evs, apodo, lista) {
    let array = [].concat(lista)
    let sumaTotal = 0
...
export function comprobarPokemones(pokemon, apodo, lista) {
    let array = [].concat(lista)
    if (pokemon == "") {
        array[2] = false
        array[4] = array[4] + " te falta un pokemon /"
        console.log("Te falta un pokemon")
    }
    return array
}

export function comprobarMovsRepetidos(movs, id, apodo, lista) {
    let array = [].concat(lista)
    let contador = 1
    for (let i = 0; i < movs.length; i++) {
        for (let x = contador; x < movs.length; x++) {
            if (movs[i] == movs[x]) {
                array[1] = false
                array[4] = array[4] + " " + apodo + " tiene un movimiento repetido."
    ...
    switch (move.secondaryEffect) {
        ...
        case "paralisys":
            if (Math.round(Math.random() * 100) <= move.probabilities && agredido.status == "" && agredido.form.type1 != "electric" && agredido.form.type2 != "electric") {
                ...
                oracion = agredido.apodo + " ahora está paralizado"
        ...
        case "freeze":
            if (Math.round(Math.random() * 100) <= move.probabilities && agredido.status == "" && agredido.form.type1 != "ice" && agredido.form.type2 != "ice") {
                ...
                console.log()
                oracion = agredido.apodo + " ahora está congelado"
                envioInforme.push(oracion)
            }
            break;
        case "burn":
            if (Math.round(Math.random() * 100) <= move.probabilities && agredido.status == "" && agredido.form.type1 != "fire" && agredido.form.type2 != "fire") {
                ...
                oracion = agredido.apodo + " ahora está quemado"
                envioInforme.push(oracion)
            }
            break;
        case "poisoned":
            if (Math.round(Math.random() * 100) <= move.probabilities && agredido.status == "" && agredido.form.type1 != "poison" && agredido.form.type2 != "poison") {
                ...
                oracion = agredido.apodo + " ahora está envenenado"
                envioInforme.push(oracion)
            }
            break;
        case "BadlyPoisoned":
            if (Math.round(Math.random() * 100) <= move.probabilities && agredido.status == "" && agredido.form.type1 != "poison" && agredido.form.type2 != "poison") {
                ...
                oracion = agredido.apodo + " ahora está gravemente envenenado"
                envioInforme.push(oracion)
            }
            break;
        case "dream":
            if (Math.round(Math.random() * 100) <= move.probabilities && agredido.status == "") {
                ...
                oracion = agredido.apodo + " ahora está dormido"
                envioInforme.push(oracion)
            }
            break;
        case "raiseAtk1":
        ...
        case "raiseDef1":
            if (Math.round(Math.random() * 100) <= move.probabilities && agresor.statsChanges[2] < 6) {
                ...
                oracion = "La defensa de " + agresor.apodo + " subio en un nivel"
                envioInforme.push(oracion)
            }
            break;
        //
...
export function turno(pkm1, pkm2, mov1, mov2, pokemonACambiar1, pokemonACambiar2, equipo1, equipo2, movPropio, movRival) {
    ...
    if (ordenTurno(pokemon1, pokemon2, mov1, mov2) == 1) {
        ...
        if (mov1 == "change") {
            oracion = pokemon1.apodo + " cambió por " + pokemonACambiar1.apodo
            envioInforme.push(oracion)
        ...
        if (mov2 == "change") {
            oracion = pokemon2.apodo + " cambió por " + pokemonACambiar2.apodo
            envioInforme.push(oracion)
